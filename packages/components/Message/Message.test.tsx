...
describe('Message', ()=>{ 
    test('message() function', async()=> {
        ...
        await rAF();
        expect(document.querySelector('.baize-message')).toBeTruthy();
        handler.close();
        await rAF();
        expect(document.querySelector('.baize-message')).toBeFalsy();
    });
    test('call message() function more than once', async () => {
        message({message: 'hello msg', duration: 0});
        message({message: 'hello msg1', duration: 0});
        await rAF();
        expect(document.querySelectorAll('.baize-message').length).toBe(2);
        closeAll();
        await rAF();
        expect(document.querySelectorAll('.baize-message').length).toBe(0);
    });
  ...
  test("message offset", async () => {
    ...
    await rAF();
    const elements = document.querySelectorAll(".baize-message");
